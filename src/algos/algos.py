...
def preprocess(initial_data, country, date):
    data = initial_data.groupby(["Country/Region", "Date"]).sum().dropna().reset_index()
...
def concat(final_data, predictions_arima, predictions_linear_regression, date):
    ...
    result_linear_regression = _convert_predictions(
        final_data, predictions_linear_regression, date, label="Linear Regression"
    )

    # time.sleep(2)
    return result_arima.merge(
        result_linear_regression, on=["Date", "Deaths"], how="outer"
    ).sort_values(by="Date")
